...
class ESPLogOSCReceiver(QMainWindow):
    ...
    def osc_voltage_handler(self, address, *args):
        ...
        if len(args) >= 2:
            ...
            # Store ESP32 timestamp for plotting
            if not hasattr(self, 'esp32_start_time'):
                self.esp32_start_time = timestamp_ms
            self.current_esp32_time = (timestamp_ms - self.esp32_start_time) / 1000.0  # Convert to seconds
        elif len(args) >= 1:
