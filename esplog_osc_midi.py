...
Requirements:
    pip3 install PyQt6 python-osc matplotlib scipy mido python-rtmidi

Usage:
    python3 esplog_osc_midi.py
...
class ESPLogOSCReceiver(QMainWindow):
    def __init__(self):
        ...
        # MIDI state
        self.midi_port = None
        self.midi_note_active = None   # currently sounding note (pitch mode)
        self.midi_thresh_note_on = False  # threshold mode note state
    ...
    def _create_midi_group(self):
        ...
        tn_info = QLabel("Uses Threshold High/Low values above.\nNote ON when V > High,\nNote OFF when V < Low.")
    ...
    def toggle_midi_port(self, enabled):
        """Open or close the virtual MIDI port"""
        if not MIDO_AVAILABLE:
            return
        ...
        if enabled:
            ...
            try:
                ...
                QMessageBox.critical(self, "MIDI Error", f"Could not open virtual MIDI port:\n{e}\n\nOn Windows, install loopMIDI first.")
        else:
    ...
    def _process_midi(self, voltage):
        """Called from handle_new_data — send CC and/or note messages"""
        if not MIDO_AVAILABLE or not self.midi_port:
            return

        ch = self.midi_channel.value() - 1  # mido is 0-indexed
        ...
        if self.midi_cc_group.isChecked():
            ...
            if self.debug_check.isChecked() and self.sample_count % 10 == 0:
                print(f"[MIDI] CC{cc_num}={cc_val} ({voltage:.3f}V)")

        # ── Notes ────────────────────────────────────────────────────
        if self.midi_note_group.isChecked():
            velocity = self.midi_velocity.value()
            mode = self.midi_note_mode.currentIndex()  # 0=threshold, 1=pitch
            ...
            if mode == 0:
                ...
                if voltage > high and not self.midi_thresh_note_on:
                    ...
                    print(f"[MIDI] Note ON {note_name(note)} ({note}) — {voltage:.3f}V > {high:.3f}V")
                elif voltage < low and self.midi_thresh_note_on:
                    self._midi_send(mido.Message('note_off', channel=ch, note=note, velocity=0))
                    self.midi_thresh_note_on = False
                    print(f"[MIDI] Note OFF {note_name(note)} ({note}) — {voltage:.3f}V < {low:.3f}V")
                ...
                if new_note != self.midi_note_active:
                    ...
                    if self.debug_check.isChecked():
                        print(f"[MIDI] Pitch → {note_name(new_note)} ({new_note}) — {voltage:.3f}V")
    ...
    def handle_new_data(self, raw, voltage):
        ...
        if MIDO_AVAILABLE and self.midi_enable.isChecked():
            self._process_midi(filtered_voltage)
    ...
    def apply_esp32_config(self):
        ...
        try:
            config_client = SimpleUDPClient(esp32_ip, 9000)
