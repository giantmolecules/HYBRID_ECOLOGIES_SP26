...
class MulticastOSCUDPServer(osc_server.ThreadingOSCUDPServer):
    """OSC server that joins a multicast group"""
    
    def __init__(self, server_address, dispatcher, multicast_group="239.0.0.1"):
...
class ESPLogOSCReceiver(QMainWindow):
    ...
    def start_listening(self):
        ...
        try:
            self.osc_server = MulticastOSCUDPServer(("0.0.0.0", port), disp, multicast_group)
            ...
            self.status_label.setText(f"Status: Listening on {multicast_group}:{port}")
            self.status_label.setStyleSheet("color: green; font-weight: bold;")
            
            print(f"Multicast OSC server started: {multicast_group}:{port}")
