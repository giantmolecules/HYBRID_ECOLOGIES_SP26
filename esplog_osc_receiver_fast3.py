...
class ESPLogOSCReceiver(QMainWindow):
    ...
    def create_controls_panel(self):
        ...
        # OSC Streaming
        stream_group = QGroupBox("OSC Streaming")
        stream_layout = QVBoxLayout()
        
        stream_layout.addWidget(QLabel("Host:"))
        self.osc_host_input = QLineEdit()
        self.osc_host_input.setText("127.0.0.1")
        stream_layout.addWidget(self.osc_host_input)
        
        stream_layout.addWidget(QLabel("Port:"))
        self.osc_port_input = QSpinBox()
        self.osc_port_input.setRange(1, 65535)
        self.osc_port_input.setValue(9000)
        stream_layout.addWidget(self.osc_port_input)
        ...
        self.stream_status_label = QLabel("Streaming: Off")
        self.stream_status_label.setStyleSheet("color: red; font-weight: bold;")
    ...
    def create_plot_panel(self):
        ...
        # Enable OpenGL for faster, smoother rendering
        self.plot_widget.useOpenGL(True)
        
        # Data curve with smooth connection
        self.curve = self.plot_widget.plot(
            pen=pg.mkPen(color='b', width=2),
            connect='finite'  # Connect all finite points
        )
    ...
    def osc_raw_handler(self, address, *args):
        ...
        if len(args) >= 2:
            ...
            self.current_raw_timestamp = args[1]
        elif len(args) >= 1:
            self.current_raw = args[0]
            self.current_raw_timestamp = None
    ...
    def osc_voltage_handler(self, address, *args):
        ...
        if len(args) >= 2:
            ...
            # Convert sample counter to time (assuming 100 Hz = 0.01s per sample)
            # We'll calculate sample rate from the data
            if not hasattr(self, 'esp32_first_sample'):
                self.esp32_first_sample = sample_counter
                self.esp32_first_time = time.time()
            
            # Calculate time based on sample number and sample rate
            samples_elapsed = sample_counter - self.esp32_first_sample
            self.current_esp32_time = samples_elapsed * 0.01  # 100 Hz = 0.01s intervals
    ...
    def handle_new_data(self, raw, voltage):
        ...
        # Add to plot buffer using ESP32 timestamp if available
        if hasattr(self, 'current_esp32_time'):
            elapsed = self.current_esp32_time
        ...
        if self.debug_check.isChecked() and self.sample_count % 10 == 0:
            print(f"Sample {self.sample_count}: Time={elapsed:.3f}s, Source={timestamp_source}, Voltage={filtered_voltage:.3f}V")
        ...
        # Stream OSC
        if self.streaming_active and self.osc_client:
            try:
                self.osc_client.send_message("/adc/ch0/raw", raw)
                self.osc_client.send_message("/adc/ch0/voltage", float(filtered_voltage))
            except Exception as e:
                print(f"OSC streaming error: {e}")
    ...
    def start_streaming(self):
        ...
        try:
            ...
            self.stream_status_label.setText(f"Streaming: On ({host}:{port})")
            self.stream_status_label.setStyleSheet("color: green; font-weight: bold;")
            print(f"OSC streaming started to {host}:{port}")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to start OSC streaming:\n{e}")
    ...
    def stop_streaming(self):
        ...
        self.stream_status_label.setText("Streaming: Off")
        self.stream_status_label.setStyleSheet("color: red; font-weight: bold;")
        ...
        if len(self.time_data) > 1:
            ...
            if self.interpolate_check.isChecked() and len(time_array) > 10:
                try:
                    ...
                    self.curve.setData(time_smooth, voltage_smooth)
                except:
                    # Fall back to regular plot if interpolation fails
